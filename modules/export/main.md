...
4. 打包为 zip：
   - 默认位置：~/Downloads/myknowledge-export-{项目名}-{日期}.zip
   - 用户指定路径 → 使用用户说的路径

5. 告知用户：
   📦 已导出「{项目名}」→ ~/Downloads/myknowledge-export-{项目名}-{日期}.zip
   📋 包含：项目状态 + 需求索引 + 导入指引
```
...
| 导入 zip 无效（无 README.md） | 提示"这不是有效的 MyKnowledge 导出包" |
