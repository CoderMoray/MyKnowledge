...
- **问题 2：Hook 自动转发用户消息** → 增加隐私保护
  ...
  - 实现：检查 `auto_record` 配置、只转发任务相关信息（最多 200 字符）、移除敏感信息
- **移除"代码量（估）"字段**
...
- **状态流转无效提示** → 列出所有合法的状态流转
  ...
  - 新增：Created/In Progress/Review/Done/Cancelled 的完整流转规则
...
- **文档更新**：USAGE.md、FAQ.md、SKILL.md、platform-detector.md、hook-guide.md、HOK.md、silent/main.md、core/main.md
...
- **问题**：安装或升级 MyKnowledge 后，第一次使用时会立刻触发更新检查（刚装好/刚升级完，不可能有更新）
- **根因**：`skill-state.yaml` 中 `update_check` 字段不存在时，视为"首次使用，需要检查"
- **修复**：添加"冷静期"规则
  - 如果 `update_check` 不存在 → 写入 `last_check: {今日}`，**跳过本次检查**
...
- 检查机制：每次调用时检查时间间隔，超过 7 天则执行检查
- 检查完成后自动更新 `last_check` 为今天日期

> 📝 版本号说明：此前误将 1.4.71、1.4.72 作为修订号发布，从 1.4.8 起恢复正常 SemVer 序列（主版本.次版本.修订号）。
